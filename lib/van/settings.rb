...
module Van
	...
	class << self
		...
		def get_config(provided_config_file = nil)
			...
			# are used.
			new_sets, dfault = {}, db_sets[:default] ||= db_defaults[:default]
		...
		def get_config_file(search_pattern = "**/db/*.kdl")
			# get file location,
		...
		def squash_settings(app)
			...
			app.options[:database] = app.options[:database] ||= {}

			host            = app.options[:database][:host]            ||=  host
			adapter         = app.options[:database][:adapter]         ||=  adapter
			database        = app.options[:database][:database]        ||=  database
			max_connections = app.options[:database][:max_connections] ||=  max_connections
			user            = app.options[:database][:user]            ||=  user
			password        = app.options[:database][:password]        ||=  password
			port            = app.options[:database][:port]            ||=  port
			logger          = app.options[:database][:logger]          ||=  logger
